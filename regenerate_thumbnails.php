<?php
// regenerate_thumbnails.php - Regenerate thumbnails for all existing images
...
// Check for action parameter in various ways
if (isset($_GET['action']) || isset($_REQUEST['action'])) {
    $isAjax = true;
} elseif (isset($_SERVER['QUERY_STRING']) && strpos($_SERVER['QUERY_STRING'], 'action=') !== false) {
    // Parse query string manually if needed
    parse_str($_SERVER['QUERY_STRING'], $queryParams);
    if (isset($queryParams['action'])) {
        $_GET['action'] = $queryParams['action'];
        $isAjax = true;
    }
}
...
// If not an AJAX request, output the HTML interface
echo "<!DOCTYPE html>
<html lang='vi'>
<head>
    <meta charset='UTF-8'>
    <meta name='viewport' content='width=device-width, initial-scale=1.0'>
    <title>Regenerate Thumbnails</title>
    <style>
...
<body>
    <div class='container'>
        ...
        <p>This script will scan all existing images and regenerate missing thumbnails.</p>
    ...
    <script>
        ...
        async function startRegeneration() {
            ...
            try {
                ...
                const scanResponse = await fetch('regenerate_thumbnails.php?action=scan');
                ...
                for (const image of scanData.images) {
                    try {
                        const response = await fetch('regenerate_thumbnails.php?action=generate', {
                            method: 'POST',
                            headers: {
        ...
        async function clearAllThumbnails() {
            ...
            try {
                const response = await fetch('regenerate_thumbnails.php?action=clear');
                const result = await response.json();
        ...
        window.addEventListener('load', async () => {
            try {
                const response = await fetch('regenerate_thumbnails.php?action=scan');
